...
CREATE TABLE IF NOT EXISTS `liga_natacao`.`Provas` (
  ...
  `sexo` ENUM('M', 'F', 'O') NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_Provas_TipoProvas1_idx` (`TipoProvas_id` ASC) VISIBLE,
  INDEX `fk_Provas_Categorias1_idx` (`Categorias_id` ASC) VISIBLE,
  CONSTRAINT `fk_Provas_TipoProvas1`
    FOREIGN KEY (`TipoProvas_id`)
...
CREATE TABLE IF NOT EXISTS `liga_natacao`.`Baterias` (
  ...
  INDEX `fk_Baterias_TipoProvas1_idx` (`TipoProvas_id` ASC) VISIBLE,
  ...
  CONSTRAINT `fk_Baterias_Eventos1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_Baterias_TipoProvas1`
    FOREIGN KEY (`TipoProvas_id`)
    REFERENCES `liga_natacao`.`TipoProvas` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
...
CREATE TABLE IF NOT EXISTS `liga_natacao`.`Inscricoes` (
  ...
  INDEX `fk_Inscricoes_Provas1_idx` (`Provas_id` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `liga_natacao`.`Eventos_Provas` (
  `Eventos_id` INT NOT NULL,
  `Provas_id` INT NOT NULL,
  PRIMARY KEY (`Eventos_id`, `Provas_id`),
