...
function editarUser(event) {
  ...
  // Atribua corretamente o ID do usuário editado
  usuarioEditado = userId; 
  console.log('usuarioEditado:', usuarioEditado); // Verificando o valor de usuarioEditado

  console.log('Valor do editando antes:', editando );
    // Ativa o modo de edição
  editando = true; 
  console.log('Valor do editando depois:', editando );
  ...
  fetch(`/admin/buscarUsuario/${userId}`)
    ...
    .then(usuario => {
      ...
      frmCadUsuario.celular.value = usuario.celular;
...
async function cadastrarUsuario() {
  ...
  if (cpfValido) {
    const usuario = {
      ...
      perfis: perfis,
      equipeId: formData.get('equipeId') || null, // Envia null se equipeId não estiver presente
      ativo: 1  // Usuário está ativo
    };
